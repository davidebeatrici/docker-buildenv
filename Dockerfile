...
RUN apt-cache --names-only search ^mxe-i686-w64-mingw32.shared* | awk '{ print $1 }' | grep -v mxe-i686-w64-mingw32.shared-qt > packages_shared.txt

RUN xargs -a packages_shared.txt apt install -y -o Dpkg::Options::="--force-overwrite"

# Install static packages
RUN apt-cache --names-only search ^mxe-i686-w64-mingw32.static* | awk '{ print $1 }' | grep -v mxe-i686-w64-mingw32.static-qt > packages_static.txt
